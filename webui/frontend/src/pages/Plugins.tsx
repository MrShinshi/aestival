      ...
      {!plugins || plugins.length === 0 ? (
        ...
        <div className="space-y-3">
          {plugins.map((p: PluginInfo) => (
            <div
              key={p.name}
              className="bg-gray-900 rounded-lg border border-gray-800 p-4 flex items-center justify-between gap-3"
            >
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-8 h-8 bg-indigo-900/40 rounded flex items-center justify-center shrink-0">
                  <Puzzle size={16} className="text-indigo-400" aria-hidden={true} />
                </div>
                <div className="min-w-0">
                  <div className="font-medium text-gray-200 truncate">{p.display_name}</div>
